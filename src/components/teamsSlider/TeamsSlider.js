...
function TeamsSlider({setSelectTeam}) {
    ...
    return (        
        <div id="teamSliderContainer">
            ...
            <p>View All</p>{
            index === 0?
            <button disabled onClick={toLeft} id="leftSlid">{"<"}</button>:
            <button onClick={toLeft} id="leftSlid">{"<"}</button>
            }
            {index === teamImages.length-12? 
            <button disabled onClick={toRight} id="rightSlid">{">"}</button>:
            <button onClick={toRight} id="rightSlid">{">"}</button>
            }
            <div id="teamSlider">                  
                <div id="teamsContainer">
                    {teamImages.map((item,index)=>{                        
                        let img = require(`../../assets/icons/${item.image}`)                      
                        return <img onClick={clickTeam} className="teamImages" src={img} key={index} name={item.name} alt={item.name +"image"}/>                
                    })}
                </div>
