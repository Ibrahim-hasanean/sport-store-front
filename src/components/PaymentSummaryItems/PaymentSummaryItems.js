...
const PaymentSummaryItems = ({item,getTotalPrice,items,index,setItems}) => {
    ...
    return (
        <div className="summaryItem">
            ...
            <div className="summaryItemTeam">
                <h3>{item.team}</h3>
                <p>{item.type} {item.category} {item.season}{item.playerName||""}</p>
            </div>
            ...
            <SummaryItemDetailes items={items} index={index} setItems={setItems} getTotalPrice={getTotalPrice} setQuantity={setQuantity} quantity={quantity} />          
