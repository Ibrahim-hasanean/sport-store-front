...
let Login = (props) => {
  ...
  let facebbokRespose = async (response) => {
    ...
    try {
      setIsLoading(true);
      let sendToken = await axios.post(`${url}/api/v1/facebooklogin`, {
  ...
  let responseGoogle = async (response) => {
    try {
      setIsLoading(true);
      let sendToken = await axios.post(`${url}/api/v1/googlelogin`, {
  ...
  return (
    <div className="login">
      <form id="form" onSubmit={submit}>
        ...
        <div>{isLoading ? <BeatLoader loading /> : <div></div>}</div>
